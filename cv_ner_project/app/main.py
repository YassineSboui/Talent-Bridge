"""
FastAPI endpoint for CV NER extraction.
...
# Model loading
# ---------------------------------------------------------------------------
MODEL_DIR = Path(__file__).parent.parent / "models" / "model-best"
...
# ---------------------------------------------------------------------------
app = FastAPI(
    title="CV NER Extraction API",
    description="Upload a PDF CV and get structured entity extraction. "
    "Supports multilingual CVs via automatic translation.",
    version="2.0.0",
)
...
async def extract_cv(
    ...
    # Text used for NER (English or original if already English / translation failed)
    ner_text = translated_text if translated_text else original_text

    # Also run NER on original text to capture language-agnostic entities
    # that translation might mangle (names, emails, tech skills)
