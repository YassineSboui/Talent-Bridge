...
Usage:
    python run_pipeline.py          # Full pipeline: clean → convert → train → serve
    python run_pipeline.py train    # Only training (data must be prepared)
    python run_pipeline.py serve    # Only start API server (model must be trained)
"""
...
def serve():
    ...
    model_path = PROJECT_ROOT / "models" / "model-best"
...
def main():
    ...
    if "train" in args:
        ...
        print("\nDone! Run 'python run_pipeline.py serve' to start the API.")
        return
    ...
    print(f"  Start API with: python run_pipeline.py serve")
