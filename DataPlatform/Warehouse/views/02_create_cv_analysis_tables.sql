...
IF OBJECT_ID(N'cv.analysis_run', N'U') IS NULL
BEGIN
...
IF OBJECT_ID(N'cv.analysis_skill', N'U') IS NULL
BEGIN
...
IF OBJECT_ID(N'cv.job_match_result', N'U') IS NULL
BEGIN
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE object_id = OBJECT_ID(N'cv.analysis_run')
      AND name = N'IX_cv_analysis_run_created_at'
)
BEGIN
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE object_id = OBJECT_ID(N'cv.job_match_result')
      AND name = N'IX_cv_job_match_result_job'
)
BEGIN
